...
async function fromImageDescriptorsOnDisk(
	...
	return new Promise<BuildTask[]>((resolve, reject) => {
		...
		extract.on('finish', () => {
			for (const pack of Object.values(taskPacks)) {
				pack.finalize();
				pack.pipe(fs.createWriteStream(path.join(outputDir, `${pack}.tar`)));
			}
			for (const task of tasks) {
				if (taskPacks[task.serviceName]) {
					// TODO: should probably be randomized to avoid collisions
					task.tarPath = path.join(outputDir, `${task.serviceName}.tar`);
