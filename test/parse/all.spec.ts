...
describe('validation', () => {
	...
	it('should throw if label `io.balena.features.requires.sw.balena-os` uses a wrong version range', async () => {
		try {
			await fs.writeFile(
				tmpPath,
				yml.dump({
	...
	it('should throw if label `io.balena.features.requires.sw.linux` uses a wrong version range', async () => {
		try {
			await fs.writeFile(
				tmpPath,
				yml.dump({
